...
def batch_extract_and_insert():
    tickers =["UCB.BR"]#get_unique_tickers() ["IES.XETRA","2FE.XETRA","CRIN.XETRA"]  # Ou get_unique_tickers()
    for ticker in tickers:
        asset_id = get_or_create_asset_id(ticker)
        update_user_watchlist_asset_id(ticker, asset_id)
        prices = fetch_daily_data_eodhd(ticker,years=10)#fetch_last_daily_data_eodhd(ticker)  # <-- ici fetch_last_daily_data_eodhd(ticker)
        if prices:
...
if __name__ == "__main__":
    ...
    check_and_alert_log("extraction_eodhd_hist", "extraction_eodhd_hist")